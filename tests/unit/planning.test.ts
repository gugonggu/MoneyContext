...
import { calculateRemainingSavings, calculateRequiredMonthlySavings, calculateSavingsProgressRatio, projectSavingsGoal } from "@/domain/savings/projection";
...
it("derives a clamped savings progress ratio", () => {
  expect(calculateSavingsProgressRatio(300_000, 700_000)).toBe(0.3);
  expect(calculateSavingsProgressRatio(1_100_000, 0)).toBe(1);
  expect(calculateSavingsProgressRatio(0, 0)).toBe(0);
});
