...
describe("transaction service", () => {
  ...
  it("rejects a transfer with neither a source nor a destination account", async () => {
    const service = createTransactionService(repository());
    await expect(service.create(userId, { type: "TRANSFER", amount: 10_000, baseAmount: 10_000, currency: "KRW", transactionAt: "2026-08-11T10:00:00+09:00" })).rejects.toThrow("at least one");
  });

  it("persists a one-sided TRANSFER out to an external party (e.g. paying a friend back)", async () => {
    const service = createTransactionService(repository());
    await expect(service.create(userId, { type: "TRANSFER", amount: 10_000, baseAmount: 10_000, currency: "KRW", transactionAt: "2026-08-11T10:00:00+09:00", fromAccountId: bank.id })).resolves.toMatchObject({ fromAccountId: bank.id, toAccountId: undefined });
  });

  it("persists a one-sided TRANSFER in from an external party (e.g. a friend paying back)", async () => {
    const service = createTransactionService(repository());
    await expect(service.create(userId, { type: "TRANSFER", amount: 10_000, baseAmount: 10_000, currency: "KRW", transactionAt: "2026-08-11T10:00:00+09:00", toAccountId: bankB.id })).resolves.toMatchObject({ fromAccountId: undefined, toAccountId: bankB.id });
  });
  ...
  it("keeps a valid owned category on a TRANSFER so it can be labeled (e.g. sending money to someone else)", async () => {
    const service = createTransactionService(repository());
    await expect(service.create(userId, { type: "TRANSFER", amount: 10_000, baseAmount: 10_000, currency: "KRW", transactionAt: "2026-08-11T10:00:00+09:00", fromAccountId: bank.id, toAccountId: bankB.id, categoryId: category.id })).resolves.toMatchObject({ categoryId: category.id });
  });

  it("rejects a category outside the current user on a TRANSFER", async () => {
    const service = createTransactionService(repository());
    await expect(service.create(userId, { type: "TRANSFER", amount: 10_000, baseAmount: 10_000, currency: "KRW", transactionAt: "2026-08-11T10:00:00+09:00", fromAccountId: bank.id, toAccountId: bankB.id, categoryId: "other-user-category" })).rejects.toThrow("active category owned");
  });
  ...
  it("manually confirms a PENDING transaction (e.g. from a REQUIRE_CONFIRMATION recurring rule)", async () => {
    const repo = repository();
    repo.confirm = async () => ({ id: "transaction-a", userId, status: "CONFIRMED", type: "EXPENSE", amount: 1, baseAmount: 1, currency: "KRW", transactionAt: "2026-08-11T00:00:00.000Z" });
    const service = createTransactionService(repo);

    await expect(service.confirm(userId, "transaction-a")).resolves.toMatchObject({ status: "CONFIRMED" });
  });

  it("rejects confirming a transaction that isn't PENDING or isn't the current user's", async () => {
    const service = createTransactionService(repository());
    await expect(service.confirm(userId, "missing")).rejects.toThrow("pending transaction not found");
  });
});
