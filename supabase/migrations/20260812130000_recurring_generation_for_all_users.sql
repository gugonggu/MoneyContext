-- Cron-driven counterpart to generate_due_recurring_transactions(date): that
-- function only ever processes the calling auth.uid()'s own rules, so a user
-- who never opens the app never gets their recurring transactions generated.
-- This function loops over every active due rule across all users and is
-- restricted to the service role, for use from a server-side cron job.
create function public.generate_due_recurring_transactions_for_all_users(input_today date)
returns table (
  rule_id uuid,
...
revoke all on function public.generate_due_recurring_transactions_for_all_users(date) from public, authenticated;
