...
생성일: 2026-08-14T06:09:42Z
기준 통화: KRW
분석 기간: 2026-08-01 ~ 2026-08-31
실제 확정 거래 집계 범위: 2026-08-01 ~ 2026-08-14
기간 상태: 진행 중
분석 목적: 이번 달 소비 분석
```

## 선택한 분석 기간 vs 실제 확정 거래 집계 범위

`분석 기간`은 사용자가 선택한 범위이고, `실제 확정 거래 집계 범위`는 그중 실제로 확정 거래가 존재하는 범위다. 확정 거래는 미래에 존재할 수 없으므로 `실제 확정 거래 집계 종료일 = min(선택한 분석 종료일, 기준일)`이며, 기준일은 `Asia/Seoul` 기준 오늘 날짜다(생성일이 UTC 타임스탬프여도 그대로 쓰지 않는다). `기간 상태`는 `진행 중`(기준일이 분석 기간 내부) 또는 `완료`(분석 기간이 기준일 이전에 끝남)로 표시하며, 아직 시작되지 않은 미래 기간을 선택한 경우 확정 거래가 없다는 점을 명확히 표시한다. 자세한 규칙은 `BUSINESS_RULES.md` 20장을 따른다.
...
고정비/변동비
예정 현금흐름
미래 지출(선택한 분석 기간과 무관, 예정 거래/미리 확정한 미래 거래/할부 잔여금)
카드 현황
저축 목표
거래 내역
데이터 해석 주의사항
...
"외부 자금 이동"의 기준은 계좌 정보의 유무가 아니라 **거래 상대편이 Money Context가 관리하는 내 금융 계정인지 여부**다. 진짜 결제 계좌 정보 누락은 "미지정"으로, 상대편이 외부(부모님, 친구, 외부 업체 등)인 송금/수신은 "외부 자금 이동"으로 구분한다. 내가 관리하는 출금 계좌를 알 수 있는 외부 송금(예: "엄마 송금 600,000원, 출금 계좌: 부산은행")은 "미지정"이나 "외부 자금 이동"이 아니라 그 계좌 이름으로 정상 집계한다. 자세한 규칙은 `BUSINESS_RULES.md` 18장을 따른다.
...
- 선택한 분석 기간이 현재 날짜 이후까지 포함하는 경우, 미래 날짜는 확정 소비 통계에 포함되지 않음
- 실제 확정 거래 집계 범위는 분석 기간과 다를 수 있으며, 월 중간 분석 결과를 완성된 월 전체 소비로 해석하면 안 됨
- 기간 잉여금은 저축 목표 적립액이 아님
- 저축 목표 적립액은 Money Context 저축 목표에 연결된 적립 내역만 의미함
- '미지정'은 결제수단(계좌) 정보 자체가 누락된 거래를 의미함
- '외부 자금 이동'은 거래 상대편이 Money Context가 관리하는 내 금융 계정이 아닌 송금/수입을 의미하며, 내가 관리하는 출금/입금 계좌 정보는 있을 수 있음
...
- '미래 지출'은 선택한 분석 기간과 무관하게 아직 실현되지 않은 모든 미래 현금흐름(예정 거래, 예정일 전에 미리 확정한 거래, 할부 잔여금)을 의미함
- 할부 잔여금은 이미 구매 시점에 지출로 전액 인식된 금액을 카드사에 갚는 미래 현금흐름이며 새로운 소비가 아님
```
...
`start_date`/`end_date`는 사용자가 선택한 분석 기간이고, `actual_data_start_date`/`actual_data_end_date`는 실제 확정 거래가 존재하는 범위다(`actual_data_end_date = min(end_date, 기준일)`, 기준일은 `Asia/Seoul` 기준). 아직 시작되지 않은 미래 기간을 선택하면 `actual_data_start_date`/`actual_data_end_date`는 `null`이고 `status`는 `NOT_STARTED`다. `status`는 `NOT_STARTED`/`IN_PROGRESS`/`COMPLETE` 중 하나다. 필드 추가만 있고 제거가 없으므로 `schema_version`은 올리지 않는다.
