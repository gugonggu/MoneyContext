...
function parseProfile(row: JsonRecord, path: string): BackupProfile { return { id: string(property(row, "id", path), `${path}.id`), display_name: string(property(row, "display_name", path), `${path}.display_name`), base_currency: currency(property(row, "base_currency", path), `${path}.base_currency`), salary_cycle_day: between(integer(property(row, "salary_cycle_day", path), `${path}.salary_cycle_day`), `${path}.salary_cycle_day`, 1, 31), timezone: string(property(row, "timezone", path), `${path}.timezone`), onboarding_completed: boolean(property(row, "onboarding_completed", path), `${path}.onboarding_completed`) }; }
