...
function mapped(map: Record<string, string>, id: string | null, path: string): string | null { if (id === null) return null; const value = map[id]; if (!value) fail(path, id); return value; }
function freshMaps(payload: BackupPayload, generateId: IdGenerator): BackupIdMaps {
  const maps = {} as BackupIdMaps;
...
export function remapBackup(input: unknown, currentUserId: string, generateId: IdGenerator = () => crypto.randomUUID()): RemappedBackup {
  ...
  const payload: BackupPayload = {
    ...
    accounts: source.accounts.map((row) => ({ ...row, id: idMaps.accounts[row.id], user_id: currentUserId, linked_account_id: mapId("accounts", row.linked_account_id, "accounts.linked_account_id") })),
    credit_card_settings: source.credit_card_settings.map((row) => ({ ...row, id: idMaps.credit_card_settings[row.id], user_id: currentUserId, account_id: mapId("accounts", row.account_id, "credit_card_settings.account_id")!, payment_account_id: mapId("accounts", row.payment_account_id, "credit_card_settings.payment_account_id")! })),
    categories: source.categories.map((row) => ({ ...row, id: idMaps.categories[row.id], user_id: currentUserId })),
    tags: source.tags.map((row) => ({ ...row, id: idMaps.tags[row.id], user_id: currentUserId })),
    transactions: source.transactions.map((row) => ({ ...row, id: idMaps.transactions[row.id], user_id: currentUserId, category_id: mapId("categories", row.category_id, "transactions.category_id"), account_id: mapId("accounts", row.account_id, "transactions.account_id"), from_account_id: mapId("accounts", row.from_account_id, "transactions.from_account_id"), to_account_id: mapId("accounts", row.to_account_id, "transactions.to_account_id"), recurring_rule_id: mapId("recurring_transactions", row.recurring_rule_id, "transactions.recurring_rule_id"), planned_transaction_id: mapId("planned_transactions", row.planned_transaction_id, "transactions.planned_transaction_id") })),
    ...
    recurring_transactions: source.recurring_transactions.map((row) => ({ ...row, id: idMaps.recurring_transactions[row.id], user_id: currentUserId, account_id: mapId("accounts", row.account_id, "recurring_transactions.account_id")!, category_id: mapId("categories", row.category_id, "recurring_transactions.category_id") })),
    planned_transactions: source.planned_transactions.map((row) => ({ ...row, id: idMaps.planned_transactions[row.id], user_id: currentUserId, account_id: mapId("accounts", row.account_id, "planned_transactions.account_id"), category_id: mapId("categories", row.category_id, "planned_transactions.category_id"), converted_transaction_id: mapId("transactions", row.converted_transaction_id, "planned_transactions.converted_transaction_id") })),
    installment_plans: source.installment_plans.map((row) => ({ ...row, id: idMaps.installment_plans[row.id], user_id: currentUserId, transaction_id: mapId("transactions", row.transaction_id, "installment_plans.transaction_id")! })),
    installment_payments: source.installment_payments.map((row) => ({ ...row, id: idMaps.installment_payments[row.id], user_id: currentUserId, installment_plan_id: mapId("installment_plans", row.installment_plan_id, "installment_payments.installment_plan_id")!, settlement_transfer_id: mapId("transactions", row.settlement_transfer_id, "installment_payments.settlement_transfer_id") })),
    monthly_budgets: source.monthly_budgets.map((row) => ({ ...row, id: idMaps.monthly_budgets[row.id], user_id: currentUserId })),
    category_budgets: source.category_budgets.map((row) => ({ ...row, id: idMaps.category_budgets[row.id], user_id: currentUserId, category_id: mapId("categories", row.category_id, "category_budgets.category_id")! })),
    savings_goals: source.savings_goals.map((row) => ({ ...row, id: idMaps.savings_goals[row.id], user_id: currentUserId })),
    savings_contributions: source.savings_contributions.map((row) => ({ ...row, id: idMaps.savings_contributions[row.id], user_id: currentUserId, goal_id: mapId("savings_goals", row.goal_id, "savings_contributions.goal_id")!, transaction_id: mapId("transactions", row.transaction_id, "savings_contributions.transaction_id"), transfer_id: mapId("transactions", row.transfer_id, "savings_contributions.transfer_id") })),
  };
