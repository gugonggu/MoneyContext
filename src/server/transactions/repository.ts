...
const map = (row: Record<string, unknown>): TransactionRecord => ({ id: String(row.id), userId: String(row.user_id), status: row.status as TransactionRecord["status"], type: row.type as TransactionRecord["type"], amount: Number(row.amount), baseAmount: Number(row.base_amount), currency: String(row.currency), transactionAt: String(row.transaction_at), accountId: row.account_id as string | undefined, fromAccountId: row.from_account_id as string | undefined, toAccountId: row.to_account_id as string | undefined, categoryId: row.category_id as string | undefined, exchangeRate: row.exchange_rate as string | undefined, memo: row.memo as string | undefined, expenseNatureUser: (row.expense_nature_user as TransactionRecord["expenseNatureUser"]) ?? undefined, expenseNatureSource: row.expense_nature_source as TransactionRecord["expenseNatureSource"] });
const payload = (input: Omit<TransactionRecord, "id" | "userId" | "status" | "expenseNatureSource"> & { expenseNatureSource?: TransactionRecord["expenseNatureSource"] }) => ({ type: input.type, amount: input.amount, base_amount: input.baseAmount, currency: input.currency, transaction_at: input.transactionAt, account_id: input.accountId ?? null, from_account_id: input.fromAccountId ?? null, to_account_id: input.toAccountId ?? null, category_id: input.categoryId ?? null, exchange_rate: input.exchangeRate ?? null, memo: input.memo ?? null, expense_nature_user: input.expenseNatureUser ?? null, expense_nature_source: input.expenseNatureSource ?? "UNSET" });
export function createTransactionRepository(supabase: SupabaseClient): TransactionRepository { return {
  ...
  async get(userId, id) { const { data, error } = await supabase.from("transactions").select("*").eq("user_id", userId).eq("id", id).maybeSingle(); if (error) throw new Error(error.message); return data ? map(data) : null; },
