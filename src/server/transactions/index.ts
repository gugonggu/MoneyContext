...
import { createTransactionService, type TransactionInput, type TransactionSearchFilters } from "@/server/transactions/service";
...
export async function createTransactionForCurrentUser(input: TransactionInput) { const { userId, service } = await current(); return service.create(userId, input); }
export async function updateTransactionForCurrentUser(id: string, input: TransactionInput) { const { userId, service } = await current(); return service.update(userId, id, input); }
